   ...
   NewSHP(MainData^.SHP, 1, 1, 1);
{
   ...
   begin
      messagebox(0, PChar('Error Palette: < ' + extractfiledir(ParamStr(0)) +
         '\Palettes\TS\unittem.pal > Not Found'), 'Palette Error', 0);
      FrmMain.Close;
   end;
}
   MainData^.SHPPaletteFilename := extractfiledir(ParamStr(0)) + '\Palettes\TS\unittem.pal';
...
begin
   ...
   begin
      ...
      Data^.SHPPaletteFilename := extractfiledir(ParamStr(0)) + '\Palettes\TS\unittem.pal';
   // Sometimes the palette choosen above might not be
   // valid, so this is a little protection against these
   // weird situations
...
begin
   ...
   if (not FrmMain.AutoSelectSHPType1.Checked) and (a <> nil) then
   begin
      Data^.SHP.SHPType := a^.SHP.SHPType;
      Data^.SHP.SHPGame := a^.SHP.SHPGame;
      Data^.SHPPaletteFilename := a^.SHPPaletteFilename;
   end
   else
   begin
      if FrmMain.PalettePreferenceData.GameSpecific then
